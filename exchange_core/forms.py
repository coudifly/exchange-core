...
class SignupForm(account.forms.SignupForm):
    ...
    password = PasswordField(
        label=_("Password"), strip=settings.ACCOUNT_PASSWORD_STRIP)
    document_1 = BRCPFField(label=_("CPF"))
    document_2 = forms.CharField(label=_("RG"))
    mobile_phone = forms.CharField(label=_("Mobile phone"))

    field_order = ['first_name', 'last_name', 'username', 'mobile_phone',  'email',
                   'confirm_email', 'password', 'password_confirm', 'code', 'document_1', 'document_2']
...
class AddressForm(forms.ModelForm):
    ...
    ), empty_label=_("-- Select your country --"), initial=3469034, required=True)
    region = forms.ModelChoiceField(
        label=_("State"), queryset=Region.objects.none(), required=True)
    city = forms.ModelChoiceField(
        label=_("City"), queryset=Region.objects.none(), required=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if country and region:
            self.fields['region'].queryset = Region.objects.filter(
                country_id=country).order_by('name')
            self.fields['city'].queryset = City.objects.filter(
                region_id=region).order_by('name')

    class Meta:
        model = Addresses
        fields = ('country', 'region', 'city', 'zipcode', 'address',
...
class UserForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user.type == Users.TYPE.company:
            self.fields['company_name'] = forms.CharField(
                label=_("Company name"))
            self.fields['branch'] = forms.CharField(
                label=_("Branch of activity"))
            self.fields['company_document_1'] = forms.CharField(
                label=_("Company number"))
            self.fields['company_document_2'] = forms.CharField(
                label=_("Company state number"))
            self.fields['manager'] = forms.CharField(label=_("Manager"))
            self.fields['comercial_phone'] = forms.CharField(
                label=_("Comercial Phone"))

    class Meta:
        model = Users
        fields = ['first_name', 'last_name', 'email',
                  'document_1', 'document_2', 'mobile_phone']
...
class BankAccountForm(forms.ModelForm):
    form_name = forms.CharField(
        widget=forms.HiddenInput(), initial='bank_account')

    class Meta:
        model = BankAccounts
        fields = ('bank', 'agency', 'agency_digit', 'account_type',
                  'account_number', 'account_number_digit',)


class ChangePasswordForm(forms.Form):
    form_name = forms.CharField(
        widget=forms.HiddenInput(), initial='change_password')
    ...
    def clean_current_password(self):
        ...
        if not self.user.check_password(current_password):
            raise forms.ValidationError(_("The current password is wrong"))
